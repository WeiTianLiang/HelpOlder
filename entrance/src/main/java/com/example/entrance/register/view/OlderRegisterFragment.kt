...
import kotlinx.android.synthetic.main.older_fragment.*
...
class OlderRegisterFragment : BaseFragment() {
    ...
    override fun onInflated(savedInstanceState: Bundle?) {
        ...
        state_register.setOnCheckedChangeListener { _, checkedId ->
            state_button = findViewById(checkedId)
            state = if(state_button?.text == "健康") {
                0
            } else {
                1
            }
    ...
    override fun getLayoutResId(): Int {
        return R.layout.older_fragment
    }
...
//                    val file = File(data.data.path)
                try {
                    ...
                    older_head.setImageBitmap(rotateImage(bitmap!!, getOrientation(context as RegisterActivity, data.data)))
